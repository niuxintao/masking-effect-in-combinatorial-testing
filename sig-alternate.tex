...
%In fact, there is a related work that care for the masking effect, however, most of its work focus on making sure the test cases generated should test each potential interactions,which ignore that the identifying process itself can also be affected by the masking effect.
...
In the second study, we want to learn how the masking effect impact on the traditional approaches. To conduct this study, we need to apply the traditional failure-inducing identifying algorithms on the SUT we collect in the first study and observe their results to compare them with the perfect combinations we got aforementioned. 
...
\subsubsection{study setup}
The traditional approaches we selected are: OFOT, FIC and CTA. To make the first two approaches (OFOT and FIC) work, they need to be fed with a failing test case. While for CTA, it works with covering array along with their execute output. As the input for the traditional approaches are different, we designed two different setups, one for OFOT and FIC, the other for CTA. 
...
We first utilize augment simulating algorithms to generate 2-way covering arrays. And for each test case in the covering array, the output of them can be fetched from the exhaustive set we have got in the first study. Then we fed the covering array along with their output to CTA approach and collected the result. As different covering array may affect the result of CTA approach, we repeated using ASA to generate 2-way covering array for 30 times and applied CTA for each of them. It is noted that ASA algorithm is a heuristic approach, which using some random factor in this algorithm, so the 30 covering array are different from each other. 

After we have collected the result got by each algorithm for each version software. Next we need to compare the result with the perfect combinations to quantify the extent to which traditional approaches suffers from masking effect. To make this comparison, we first need to introduce the following notation:
...
\subsubsection{result and discuss}
Table \ref{traiditional_result} depicts the result of the second case study. Column "Num Diff" indicate the discrepancy of the number of MFS between the ones got through traditional approaches with the perfect combinations. Column "Similarity" indicate the similarity between this two set of MFS. Additionally, the results of different algorithms can be distinguished by the titles of sub-columns, which are "FIC","OFOT","CTA", indicating  corresponding algorithms respectively. It is noted that for approach "CTA", the vale we set in the cell are the average value of the result we collected from 30 repeated experiments' result for a particular subject.
...
From this table, we can easily find that Traditional MFS identifying approaches do suffer from the multiple faults and their masking effect. For example, for the Grep, version 3.14, all these three algorithm will lost some information. Additional, the extent to which the impact has affected vary from algorithms. We can find that for HSQLDB with version 3.14, the similarity of CTA was just at 4 while FIC is 3. We list these points in figure 3. From this figure, we can learn that.

So the answer to the second question is: traditional algorithm do suffer from the multiple faults and their masking effect although the degree of impact vary in different algorithms.
...
\subsubsection{study setup}
The setup of this case study is almost the same as study 2. The difference is that the algorithms we choose are the ones augment with our approach.  Additionally, we will the comparison of this two case study will also be quantified. 
