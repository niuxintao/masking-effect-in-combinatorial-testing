...
Q1: What the condition of faults and their masking effect in real software.

Q2: What the behaviour of traditional approaches works on software when they have multiple faults?

Q3: Does our approach get a better solution than traditional approaches when facing multiple faults?

Specifically, section 6.1 survey several open-source software to gain a insight of the condition of multiple faults and their masking effects. Section 6.2 applied three MFS-identifying programs on the faults-controllable software from the SIR benchmark. Section 6.3 apply our approach on the benchmark and a comparison with traditional approaches will be discussed. Section 6.4 discuss the threats to validity of our empirical studies.

\subsection{study 1: multiple faults and masking effects in practice}
In the first study, we survey several software to gain a insight of the condition of multiple faults and their effects. The software under study are four GNU software: Grep, Sed, Make, Gzip and a database management software- HSQLDB. Each of them contain different versions. The reason why we choose them as the subject is that these software are highly configurable, the options and their combination can influence their behaviour, additionally, they all have the developer community so that we can easily get the real bugs reported in the bug tracker forum. Table \ref{software description} lists the program, the number of versions we surveyed, number of lines of uncommented code, number of procedures(for c software) or number of classes (for java software) and a brief description.
...
The number of masking effect among the bugs for a specific version will be given in the next section.
...
software & versions & Num of faults & Num of masking\\ \hline
Grep &- &  49  &-\\ \hline
Sed & - & -  & - \\ \hline
Make & - &  131 &-\\ \hline
Gzip &- &  - &-\\ \hline
...
\emph{grep --only-matching --count '\textbackslash<²ð\textbackslash>' test.txt} 
...
\subsubsection{study setup}
For each version of software, we do the following: First use the TSL describe the model of software, generate test cases and run them. Remodel them as Cohen do. The select the failing test cases,and use algorithms to identify the result. And then use multiple faults, do the same as previous said, then compared the results as the followed
...
To evaluate our approach, we take two open-source software as our subjects: GNU software: Grep,Sed,Make, and so on. HSQLDB and Log4j. HSQLDB is a pure-java data-base management software, and Log4j is a logger tool commonly used in java application. Both of them have a large support developers' forum. Furthermore as our approach is a framework that can bee seen as a for the MFS identifying algorithms, so that we afford them five algorithms: fic ofot trt cta sp.
