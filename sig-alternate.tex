...
\sout{(  -  \ \ \  0 \ \ \  0  \ \ \ 0 ) \ \ \ } \uwave{(  -  \ \ \  - \ \ \  0  \ \ \ 0 ) \ \ \ for Err 1} \\
\hline
\end{tabular}
\end{table}

\subsubsection{CTA examples}
CTA uses the covering array as its inputs and then use classification tree algorithm to characterize the MFS. For this algorithm, we still assume the SUT has 4 parameters and each one has 3 values. Then we will  For CTA, usually it is a covering array. IF we cannot change it with pass or expected fault, we just discard them as they can't be the schemas for the current fault.
