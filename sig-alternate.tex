...
\maketitle
\begin{abstract}
Combinatorial testing(CT) is proven to be effective to reveal the potential failures caused by the interaction of the inputs or options of the system under test(SUT). A key problem in CT is to isolate the failure-inducing interactions in SUT as it can facilitate the debugging effort by reducing the scope of code that needs to be inspected. Many algorithms has been proposed to identify the failure-inducing interactions in SUT, however, most of these studies either just consider the condition of one fault during testing or ignore masking effects among multiple faults which can bias their identified results. In this paper, we analysed how the masking effect of multiple faults impact on the failure-inducing interactions identifying results. We further give an strategy of replacing test cases during testing to alleviate this impact. This replacement process first eliminate the test cases that may trigger masking effect and then repeat generating new test cases until we get some no-masking-effect test cases which can compensate for the information loss of these eliminated test cases. We conducted some empirical studies on several open-source gnu software, and the result shows that multiple faults do exist in real software and our approach can assist combinatorial-based failure-inducing identifying methods to get a better result when dealing multiple faults in SUT.
\end{abstract}
...
With the increasing complexity and size of modern software, many factors, such as input parameters and configuration options, can influence the behaviour of the system under test(SUT). The unexpected faults caused by the interaction among these factors can make testing such software a big challenge if the interaction space is large. Combinatorial testing can systematically sample the interaction space and detect the possible failure-inducing iteration with the strength of the interaction no more than \emph{t}. Furthermore, many combinatorial-based fault location algorithms are proposed to locate the failure-inducing interaction of the SUT. However, in our recently studies on several real software, we find these methods didn't behave as expected when these software contain multiple faults. The main reason why these methods fails to behave normally is that they didn't consider the masking effect that may happens among different faults. In this paper, we first analysis the problems when locating failure-inducing interactions of SUTs with multiple faults in theory, and then give a solution that can alleviate the impact of masking effect of different faults. This method separately handle each fault, and for a particular fault, it will eliminate the test cases that trigger other faults and replace them with test cases either pass or trigger the same fault as currently analysis. We evaluate our approach in several open-source gnu projects, and the result of the empirical studies shows that multiple faults do exist in real software and our approach can assist combinatorial-based failure-inducing identifying methods to get a better result when facing multiple faults in SUT.
