...
\maketitle
\begin{abstract}
With the increasing complexity and size of modern systems, the interaction of many factors, such as input parameters and configuration options(we call them interaction space), can influence the behaviour of the system under test(SUT). To isolate these interactions that trigger faults in the SUT is a challenge if the interaction space is large. Combinatorial testing can systematically sample the interaction space and detect the possible failure-inducing iteration with the strength of the interaction no more than \emph{t}. Furthermore, many combinatorial-based fault location algorithms are proposed to locate the failure-inducing interaction of the SUT. However, in our recently studies in several real software, we find these methods didn't behave as expected as these software contain multiple faults. The reason why these methods fails to behave normally is that they didn't consider the masking effect that may happens among different faults. In this paper, we first analysis the problems when identify failure-inducing interactions in a SUT with multiple faults in theory, and then give a solution that can alleviate the impact of masking effect of different faults. This method separately handle each fault, and for a particular fault, it will eliminate the test cases that trigger other faults and replace them with test cases either pass or trigger the same faults as currently analysis. We evaluate our approach in several open-source gnu projects, and the result of the empirical studies shows that multiple faults do exist in real software and our approach can assist combinatorial-based failure-inducing identifying methods to get a better result when facing multiple faults in SUT.
\end{abstract}
