...
for SUT(3,2), Assume (1,1,0) is the MFS of err 1 and (1,1,1) is the MFS of err 2, The test configurations and result is listed in table \ref{example_first_scenario}.
...
And then we will wrongly identified as (1,1,-) for the fault.

\subsection{A simple idea : do not consider the masking effect}
...
Using this we will treat the table \ref{example_first_scenario} as the following table \ref{simple_idea}, then we will get the result as : (1,1,0) for err 1 and (1,1,1) err 2 which coincide with the result as initial assumption.

\begin{table}
\centering
\caption{simple idea}
\label{simple_idea}
\begin{tabular}{p{0.39\columnwidth}|p{0.4\columnwidth}} \hline
   ERR 1 & ERR 2
\end{tabular}

\begin{tabular}{c|c|c|c|c|c} \hline
id &test inputs & result & id&test inputs & result\\ \hline
1 &(0, 0, 0) &  PASS &1&(0, 0, 0) &  PASS\\ \hline
2 &(0, 0, 1) &  PASS &2&(0, 0, 1) &  PASS\\ \hline
3 &(0, 1, 0) &  PASS &3&(0, 1, 0) &  PASS\\ \hline
4 &(0, 1, 1) &  PASS &4&(0, 1, 1) &  PASS\\ \hline
5 &(1, 0, 0) &  PASS &5&(1, 0, 0) &  PASS\\ \hline
6 &(1, 0, 1) &  PASS &6&(1, 0, 1) &  PASS\\ \hline
7 &(1, 1, 0) &  FAIL &7&(1, 1, 0) &  PASS\\ \hline
8 &(1, 1, 1) &  PASS &8&(1, 1, 1) &  FAIL\\ \hline
\hline\end{tabular}
\end{table}


However it also didn't completely solve the problem as it didn't consider the masking problem. As for a particular fault, say $F_(i)$, we didn't know whether the test configurations trigger a higher fault than $F_{i}$, say, $F_{j}, j \leq i$ will or not trigger $F_{i}$, if the $F_{j}$ is not triggered. The only information we know is that the test configuration triggered a lower fault, say, $F_{k}, k \geq i$ will not trigger the fault $F_{i}$.

So if masking effect existed in SUT, we will incorrectly get the result. For example, If we have SUT(3,2) and (1,1,-) for err 1 and (-,-,0) for err 2. And err 1 has higher level than err 2.  The test configurations and the result is shown in table \ref{example_simple}.
\begin{table}
\centering
\caption{masking effect example}
\label{example_simple}
\begin{tabular}{|c|c|c|} \hline
id&test inputs & result\\\hline
1&(0, 0, 0) &  Err 2\\ \hline
2&(0, 0, 1) &  PASS\\ \hline
...
Then if we do not consider the level of faults as the simple idea do we will treat the test configuration and result as table \ref{simple_idea_masking}. Then we will regard (1,1,-) is the MFS for err 1 and (0,-,0),(-,0,0) for err 2.

We incorrectly identified the MFS for err 2 because the test configuration (1,1,0) masked the schema (-,-,0) which should trigger err 2 but as it triggered a higher fault err 1, so we defaulted consider it did not trigger err 2.

\begin{table}
\centering
\caption{simple idea for masking effect example}
\label{simple_idea_masking}
\begin{tabular}{p{0.39\columnwidth}|p{0.4\columnwidth}} \hline
   ERR 1 & ERR 2
\end{tabular}
...
\begin{tabular}{c|c|c|c|c|c} \hline
id &test inputs & result & id&test inputs & result\\ \hline
1 &(0, 0, 0) &  PASS &1&(0, 0, 0) &  FAIL\\ \hline
2 &(0, 0, 1) &  PASS &2&(0, 0, 1) &  PASS\\ \hline
3 &(0, 1, 0) &  PASS &3&(0, 1, 0) &  FAIL\\ \hline
4 &(0, 1, 1) &  PASS &4&(0, 1, 1) &  PASS\\ \hline
5 &(1, 0, 0) &  PASS &5&(1, 0, 0) &  FAIL\\ \hline
6 &(1, 0, 1) &  PASS &6&(1, 0, 1) &  PASS\\ \hline
7 &(1, 1, 0) &  FAIL &7&(1, 1, 0) &  PASS\\ \hline
8 &(1, 1, 1) &  FAIL &8&(1, 1, 1) &  PASS\\ \hline
\hline\end{tabular}
\end{table}
...
We can see the different part with the previous formula is that it just minus the schemas in these passing test configurations and these test configurations triggering a lower level than $F_{i}$. With this, we will treat the table \ref{example_simple} as : table \ref{another_idea_masking}. And we can easily get the expected result:(1,1,-) for Err 1 and (-,-,0) for Err 2.

\begin{table}
\centering
\caption{Another idea for masking effect example}
...
5 &(1, 0, 0) &  PASS &5&(1, 0, 0) &  FAIL\\ \hline
6 &(1, 0, 1) &  PASS &6&(1, 0, 1) &  PASS\\ \hline
7 &(1, 1, 0) &  FAIL &7&(1, 1, 0) &  FAIL\\ \hline
8 &(1, 1, 1) &  FAIL &8&(1, 1, 1) &  PASS\\ \hline
\hline\end{tabular}
\end{table}


But it still not a perfect solution, obviously not, because not all the test configurations triggering higher level fault will trigger lower level fault. Consider the following example, for SUT(3,2), Assume (1,-,-) is the MFS of err 1 and (-,1,1) is the MFS of err 2, And err 1 has higher level than err 2. The test configurations and result is listed in table \ref{partially_example}.

\begin{table}
\centering
\caption{partially masking effect example}
\label{partially_example}
\begin{tabular}{|c|c|c|} \hline
id&test inputs & result\\\hline
1&(0, 0, 0) &  PASS\\ \hline
2&(0, 0, 1) &  PASS\\ \hline
...
If we using the computing model listed in this subsection, we will find (1,-,-) for err 1 and then (1,-,-) and (-,1,1) for Err 2.  So why do this happened? The reason is listed in table \ref{another_idea_partially_masking}. In this table, we find that for the 5th , 6th and 7th test configuration, these should not trigger the err 2, but we defaulted set them as triggered err 2 because they have trigger a higher level fault err 1. So this lead us a wrong process to identify the MFS for err 2.

\begin{table}
\centering
\caption{Another idea for partially masking effect example}
\label{another_idea_partially_masking}
\begin{tabular}{p{0.39\columnwidth}|p{0.4\columnwidth}} \hline
   ERR 1 & ERR 2
\end{tabular}
...
2 &(0, 0, 1) &  PASS &2&(0, 0, 1) &  PASS\\ \hline
3 &(0, 1, 0) &  PASS &3&(0, 1, 0) &  PASS\\ \hline
4 &(0, 1, 1) &  PASS &4&(0, 1, 1) &  FAIL\\ \hline
5 &(1, 0, 0) &  FAIL &5&(1, 0, 0) &  FAIL\\ \hline
6 &(1, 0, 1) &  FAIL &6&(1, 0, 1) &  FAIL\\ \hline
7 &(1, 1, 0) &  FAIL &7&(1, 1, 0) &  FAIL\\ \hline
8 &(1, 1, 1) &  FAIL &8&(1, 1, 1) &  FAIL\\ \hline
\hline\end{tabular}
\end{table}
...
And some test configurations in this set will trigger $F_{i}$. We label them as:
...
Let $$S_{ca} = S_{T_{tri-F_{i}}} S_{T_{F_{i}}} + S_{T_{F_{i}}} - S_{T_{P}} -  S_{T_{\neg tri-F_{i}}} $$

$$M_{F} = \{s_{i} | s_{i} \in S_{ca}\ and \not\exists s_{j} \prec s_{i}\ s.t.\ s_{j} \in S_{ca} \}$$


However this formula is impractical, because we cannot make sure these schemas will not trigger lower fault except we having removing the higher level fault and re-executing this test configurations.
