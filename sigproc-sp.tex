...
Following we have construct a example to illustrate the motivation of our approach. Assume we have a method \emph{foo} which has four input parameters : \emph{a, b, c, d}. The types of these four parameters are all integers and the values that they can take are: d_{a} = \{7, 11\}, d_{b} = \{2, 5\}, d_{c} = \{2, 6\}, d_{b} = \{3, 6\} respectively.  The detail code of this method is listed as following:
...
However, when we use traditional algorithms to apply in this example, we can find the result is not this t. Consider the followed test case, yes, it failed with, however, when we change a value of it to another , is it fails, but with different error message. So this time how can we .
if else 
...
